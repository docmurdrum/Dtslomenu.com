...
function renderCharacterCard() {
  ...
  const { character, sceneIndex, charIndex } = getCharacterForLevel(level);

  const scene = character.scenes[sceneIndex];
  document.getElementById('char-tier-badge').textContent = `Character ${charIndex + 1} of 50 · Scene ${sceneIndex + 1}`;
  document.getElementById('char-name').textContent       = character.name;
  document.getElementById('char-archetype').textContent  = scene.action;

  const tc = document.getElementById('char-traits');
  tc.innerHTML = `<div class="char-trait">${scene.bg}</div>`;
...
// ── LOAD CHARACTER FROM STORAGE (with live-gen fallback) ──
async function generateCharacter() {
  ...
  wrap.innerHTML = `<div class="char-generating"><div class="spinner"></div><span style="font-size:12px;color:var(--text2)">Loading your character…</span></div>`;

  // ── Try Supabase Storage first (pre-generated) ──
  const storageUrl = getCharacterStorageUrl(level);
  try {
    const testRes = await fetch(storageUrl, { method: 'HEAD' });
    if (testRes.ok) {
      // Image exists in storage — load it instantly
      wrap.innerHTML = `<img id="char-img" src="${storageUrl}" alt="Your character" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.parentElement.innerHTML='<div class=\\'char-img-placeholder\\'><div>❌</div><span>Image failed to load</span></div>'">`;
      document.getElementById('char-share-btn').style.display = 'inline-flex';
      showToast('🎨 Character loaded!');
      btn.disabled = false; btn.textContent = '🔄 Regenerate';
      return;
    }
  } catch(e) { /* storage not available, fall through to live gen */ }

  // ── Fallback: live generation via Cloudflare Worker ──
  ...
  const { sceneIndex: lvlScene } = getCharacterForLevel(newLevel);
  const prompt = buildPrompt(character, lvlScene);
  ...
  try {
    ...
    if (blob.size < 1000) throw new Error('Bad response from generator');
